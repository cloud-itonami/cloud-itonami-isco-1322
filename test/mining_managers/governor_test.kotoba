...
(deftest accepts-clean-low-stake-proposal
  (let [st (fresh-store)
        request {:manager-id "manager-1"}
        proposal {:op :schedule-shift :effect :propose :confidence 0.95}
...
(deftest hard-blocks-unregistered-manager
  (let [st (fresh-store)
        request {:manager-id "no-such-manager"}
        proposal {:op :schedule-shift :effect :propose :confidence 0.95}
        verdict (governor/check request {} proposal st)]
    (is (not (:ok? verdict)))
    (is (:hard? verdict))
...
(deftest escalates-high-risk-site-operation
  (let [st (store/mem-store)
        _ (store/register-manager! st {:manager-id "manager-1" :name "Alice Chen" :certifications ["MSHA"] :experience-years 12})
        _ (store/register-site! st {:site-id "site-high-risk" :operator-id "operator-1" :location "Nevada" :risk-level :high})
        request {:manager-id "manager-1" :site {:site-id "site-high-risk"}}
        proposal {:op :schedule-shift :effect :propose :confidence 0.95}
        verdict (governor/check request {} proposal st)]
    (is (not (:ok? verdict)))
    (is (not (:hard? verdict)))
...
(deftest escalates-low-confidence-proposal
  (let [st (fresh-store)
        request {:manager-id "manager-1"}
        proposal {:op :log-production-report :effect :propose :confidence 0.5}
        verdict (governor/check request {} proposal st)]
    (is (not (:ok? verdict)))
    (is (not (:hard? verdict)))
    (is (:escalate? verdict))))
...
(deftest hard-blocks-unregistered-site
  (let [st (store/mem-store)
        _ (store/register-manager! st {:manager-id "manager-1" :name "Alice Chen" :certifications ["MSHA"] :experience-years 12})
        request {:manager-id "manager-1" :site {:site-id "unknown-site"}}
...
(deftest hard-blocks-non-propose-effect
  (let [st (fresh-store)
        request {:manager-id "manager-1"}
        proposal {:op :schedule-shift :effect :commit :confidence 0.95}
        verdict (governor/check request {} proposal st)]
    (is (not (:ok? verdict)))
    (is (:hard? verdict))
    (is (seq (:violations verdict)))))
